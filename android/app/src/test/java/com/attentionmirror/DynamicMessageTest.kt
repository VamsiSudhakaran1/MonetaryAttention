...
class DynamicMessageTest {
    ...
    fun `same day produces a stable message`() {
        ...
        val a = DynamicMessages.forDay(receipt(100.0), 80.0, "9 PM", date, Tone.GENTLE)
        val b = DynamicMessages.forDay(receipt(100.0), 80.0, "9 PM", date, Tone.GENTLE)
        assertEquals(a, b)
    }
    ...
    fun `message changes across days`() {
        ...
        val messages = (0..6).map {
            DynamicMessages.forDay(r, 80.0, "9 PM", LocalDate.of(2026, 6, 17).plusDays(it.toLong()), Tone.GENTLE)
        }
    ...
    fun `no leftover placeholders in any tone`() {
        ...
        for (tone in Tone.values()) {
            listOf(0.0, 10.0, 100.0, 240.0).forEach { mins ->
                val m = DynamicMessages.forDay(receipt(mins), 50.0, "9 PM", date, tone)
    ...
    fun `hard truth changes the body`() {
        val date = LocalDate.of(2026, 6, 17)
        val soft = DynamicMessages.forDay(receipt(100.0), 80.0, "9 PM", date, Tone.GENTLE)
        val hard = DynamicMessages.forDay(receipt(100.0), 80.0, "9 PM", date, Tone.HARD)
    ...
    fun `quirky differs from gentle`() {
        val date = LocalDate.of(2026, 6, 17)
        val gentle = DynamicMessages.forDay(receipt(100.0), 80.0, "9 PM", date, Tone.GENTLE)
        val quirky = DynamicMessages.forDay(receipt(100.0), 80.0, "9 PM", date, Tone.QUIRKY)
        assertNotEquals(gentle.headline, quirky.headline)
    }

    @Test
    fun `no usage uses the empty bucket`() {
        val m = DynamicMessages.forDay(receipt(0.0, low = 0, high = 0, ads = 0), null, null, LocalDate.of(2026, 6, 17), Tone.GENTLE)
